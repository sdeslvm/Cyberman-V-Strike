...
protocol WebLoadable: AnyObject {
    var state: CybermanWebStatus { get set }
...
final class CybermanWebLoader: NSObject, ObservableObject, WebLoadable, ProgressMonitoring {
    // MARK: - Свойства

  @Published var state: CybermanWebStatus = .standby
    ...
    func observeProgression() {
        progressPublisher
            ...
            .sink { [weak self] progress in
                ...
                let newState: CybermanWebStatus = progress < 1.0 ? .progressing(progress: progress) : .finished
...
extension CybermanWebLoader: WKNavigationDelegate {
...
extension CybermanWebLoader {
    ...
    func publish(_ newState: CybermanWebStatus) {
...
private extension CybermanWebLoader {
    func updateState(_ newState: CybermanWebStatus) {
