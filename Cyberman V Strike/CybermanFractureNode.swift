...
protocol CybermanLoadStateRepresentable {
    var type: CybermanLoadState.StateType { get }
...
struct CybermanLoadState: Equatable, CybermanLoadStateRepresentable {
    ...
    static func idle() -> CybermanLoadState {
        CybermanLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> CybermanLoadState {
        CybermanLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> CybermanLoadState {
        CybermanLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> CybermanLoadState {
        CybermanLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> CybermanLoadState {
        CybermanLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: CybermanLoadState) -> Bool {
    ...
    static func == (lhs: CybermanLoadState, rhs: CybermanLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension CybermanLoadState: CustomStringConvertible {
