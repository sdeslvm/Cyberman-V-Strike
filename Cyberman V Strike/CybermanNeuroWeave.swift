...
struct CybermanWebViewBox: UIViewControllerRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: CybermanWebLoader
    var defaultOrientations: UIInterfaceOrientationMask = .landscape

    // MARK: - Координатор

    func makeCoordinator() -> CybermanWebCoordinator {
        CybermanWebCoordinator { [weak loader] status in
    ...
    func makeUIViewController(context: Context) -> CybermanBaseWebViewController {
        let configuration = createWebViewConfiguration()
        return CybermanBaseWebViewController(
    ...
    func updateUIViewController(_ uiViewController: CybermanBaseWebViewController, context: Context) {
