...
struct CybermanGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://cybervstrike.pro/info")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            CybermanEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    CybermanGameInitialView()
}
